...
# The model includes predictor variables and the logarithm of PFI time as covariates
fit = survreg(Surv(train_data$OS.time, train_data$OS) ~ X + log(train_data$PFI.time), dist = "weibull")
...
# Calculate the risk score for the test data using the Weibull regression model coefficients,
# including the logarithm of PFI time as an additional predictor
test_data[, 20] = -cbind(X, log(test_data$PFI.time)) %*% coefficients

# Perform time-dependent ROC analysis on the test data
ROC.regression2 <- timeROC(T = test_data$OS.time,
                           delta = test_data$OS,
...
plotAUCcurve(ROC.regression2, conf.int = TRUE, col = "aquamarine")
