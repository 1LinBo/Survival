...
Exponential_predict = function(y) {
    lambda1 = exp(y[1])  # Exponential rate for PFI
    lambda2 = exp(y[2])  # Exponential rate for OS
    theta = exp(y[3])  # Shared parameter between PFI and OS
    beta1 = y[(3 + 1):(3 + p)]  # Coefficients for PFI predictors
    beta2 = y[(3 + p + 1):(3 + 2 * p)]  # Coefficients for OS predictors
    ...
    r2 = as.vector(lambda2)  # Baseline hazard for OS
    R1 = as.vector(T1 * lambda1)  # Baseline cumulative hazard for PFI
    R2 = as.vector(T2 * lambda2)  # Baseline cumulative hazard for OS
...
repeat {
    ...
    res_Random = try(optim(p0, Exponential_predict, method = "BFGS", hessian = TRUE), silent = TRUE)  # Perform optimization
...
lambda1 = exp(res$par[1])  # Exponential rate for PFI
lambda2 = exp(res$par[2])  # Exponential rate for OS
theta = exp(res$par[3])  # Shared parameter
beta1 = res$par[(3 + 1):(3 + p)]  # Coefficients for PFI predictors
beta2 = res$par[(3 + p + 1):(3 + 2 * p)]  # Coefficients for OS predictors
...
R1 = as.vector(test_data$PFI.time * lambda1)  # Baseline cumulative hazard for PFI in the test data
...
ROC.exponential <- timeROC(T = test_data$OS.time,
                           delta = test_data$OS,
...
plotAUCcurve(ROC.exponential, conf.int = TRUE, col = "aquamarine")
