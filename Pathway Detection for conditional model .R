...
# Piecewise_Exponential model
Piecewise_Exponential_LRT = function (T1, event1, T2, event2, X){
	X = as.matrix(X) # Convert to matrix form
	p = ncol(X) # number of variables
    ...
    likelihood_H0 = function(y) {
    	...
    	g2 = exp(y[7:12])  # Exponential rates for OS in each time interval
    	theta = exp(y[13])  # Shared parameter between PFI and OS
    	beta1 = 0  # Coefficients for PFI predictors is 0
    	beta2 = 0  # Coefficients for OS predictors is 0
    	l = 0  # Initialize log-likelihood
    	X1 = as.vector(X %*% beta1)  # Linear combination for PFI
    	X2 = as.vector(X %*% beta2)  # Linear combination for OS
	...
	MPL = 0
	repeat {
    		p0 = runif(13, -1, 1)  # Generate random initial parameters
    		res_Random = try(optim(p0, likelihood_H0, method = "BFGS", hessian = TRUE), silent = TRUE)  # Perform optimization
    		if (class(res_Random) != "try-error") {  # Check if optimization was successful
        		if (res_Random$convergence == 0 & (min(eigen(res_Random$hessian)$values) > 0)) {
            		res = res_Random  # Save the result if it converged and the Hessian is positive definite
...
para3=NULL
tes= Piecewise_Exponential_LRT(T1=data$PFI.time,event1=data$PFI,T2=data$OS.time,event2=data$OS,X=data$WNT)
	P3=1-pchisq(tes[1],df=2)
	beta1=tes[7]
	beta2=tes[8]
	para3=tes[2:16]
	if(beta2>=0 & beta1<=beta2){
