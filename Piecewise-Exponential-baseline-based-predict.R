...
# Define quantiles for PFI and OS times for piecewise exponential model
a1 = quantile(T1)[1]  # 0th percentile (min value) for PFI
a2 = quantile(T1)[2]  # 25th percentile for PFI
a3 = quantile(T1)[3]  # 50th percentile (median) for PFI
a4 = quantile(T1)[4]  # 75th percentile for PFI
a5 = quantile(T1)[5]  # 100th percentile (max value) for PFI
b1 = quantile(T2)[1]  # 0th percentile (min value) for OS
b2 = quantile(T2)[2]  # 25th percentile for OS
b3 = quantile(T2)[3]  # 50th percentile (median) for OS
b4 = quantile(T2)[4]  # 75th percentile for OS
b5 = quantile(T2)[5]  # 100th percentile (max value) for OS

# Define a function to calculate the negative log-likelihood for the piecewise exponential model
Piecewise_Exponential_predict = function(y) {
    g1 = exp(y[1:6])  # Exponential rates for PFI in each time interval
    g2 = exp(y[7:12])  # Exponential rates for OS in each time interval
    theta = exp(y[13])  # Shared parameter between PFI and OS
    beta1 = y[(13 + 1):(13 + p)]  # Coefficients for PFI predictors
    beta2 = y[(13 + p + 1):(13 + 2 * p)]  # Coefficients for OS predictors
    ...
    # Baseline hazard for OS
    r2 <- ifelse(T2 < b1, g2[1], ifelse(T2 < b2, g2[2], ifelse(T2 < b3, g2[3], ifelse(T2 < b4, g2[4], ifelse(T2 < b5, g2[5], g2[6])))))
    
    # Baseline cumulative hazard for PFI
    R1 <- ifelse(T1 < a1, g1[1] * T1, ifelse(T1 < a2, g1[1] * a1 + g1[2] * (T1 - a1), 
                  ifelse(T1 < a3, g1[1] * a1 + g1[2] * (a2 - a1) + g1[3] * (T1 - a2), 
                  ifelse(T1 < a4, g1[1] * a1 + g1[2] * (a2 - a1) + g1[3] * (a3 - a2) + g1[4] * (T1 - a3), 
                  ifelse(T1 < a5, g1[1] * a1 + g1[2] * (a2 - a1) + g1[3] * (a3 - a2) + g1[4] * (a4 - a3) + g1[5] * (T1 - a4), 
                  g1[1] * a1 + g1[2] * (a2 - a1) + g1[3] * (a3 - a2) + g1[4] * (a4 - a3) + g1[5] * (a5 - a4) + g1[6] * (T1 - a5))))))
    
    # Baseline cumulative hazard for OS
    R2 <- ifelse(T2 < b1, g2[1] * T2, ifelse(T2 < b2, g2[1] * b1 + g2[2] * (T2 - b1), 
                  ifelse(T2 < b3, g2[1] * b1 + g2[2] * (b2 - b1) + g2[3] * (T2 - b2), 
                  ifelse(T2 < b4, g2[1] * b1 + g2[2] * (b2 - b1) + g2[3] * (b3 - b2) + g2[4] * (T2 - b3), 
                  ifelse(T2 < b5, g2[1] * b1 + g2[2] * (b2 - b1) + g2[3] * (b3 - b2) + g2[4] * (b4 - b3) + g2[5] * (T2 - b4), 
                  g2[1] * b1 + g2[2] * (b2 - b1) + g2[3] * (b3 - b2) + g2[4] * (b4 - b3) + g2[5] * (b5 - b4) + g2[6] * (T2 - b5))))))
...
repeat {
    p0 = runif(13 + 2 * p, -1, 1)  # Generate random initial parameters
    res_Random = try(optim(p0, Piecewise_Exponential_predict, method = "BFGS", hessian = TRUE), silent = TRUE)  # Perform optimization
...
g1 = exp(res$par[1:6])  # Exponential rates for PFI in each time interval
g2 = exp(res$par[7:12])  # Exponential rates for OS in each time interval
theta = exp(res$par[13])  # Shared parameter
beta1 = res$par[(13 + 1):(13 + p)]  # Coefficients for PFI predictors
beta2 = res$par[(13 + p + 1):(13 + 2 * p)]  # Coefficients for OS predictors
...
# Baseline cumulative hazard for PFI in the test data
T1 = test_data$PFI.time
R1 <- ifelse(T1 < a1, g1[1] * T1, ifelse(T1 < a2, g1[1] * a1 + g1[2] * (T1 - a1), 
              ifelse(T1 < a3, g1[1] * a1 + g1[2] * (a2 - a1) + g1[3] * (T1 - a2), 
              ifelse(T1 < a4, g1[1] * a1 + g1[2] * (a2 - a1) + g1[3] * (a3 - a2) + g1[4] * (T1 - a3), 
              ifelse(T1 < a5, g1[1] * a1 + g1[2] * (a2 - a1) + g1[3] * (a3 - a2) + g1[4] * (a4 - a3) + g1[5] * (T1 - a4), 
...
ROC.piecewise_exponential <- timeROC(T = test_data$OS.time,
                                     delta = test_data$OS,
...
plotAUCcurve(ROC.piecewise_exponential, conf.int = TRUE, col = "aquamarine")
